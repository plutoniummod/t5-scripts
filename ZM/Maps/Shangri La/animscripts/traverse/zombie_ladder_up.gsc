...
//	self traverseMode("nogravity");
	...
	startAnim = %ai_zombie_ladder_climb_on;
	...
	self animscripts\traverse\zombie_shared::TraverseStartRagdollDeath();

	self setFlaggedAnimKnoballRestart("climbanim",startAnim, %body, 1, .1, 1);
	self animscripts\zombie_shared::DoNoteTracks("climbanim");
	...
	if (climbingTime > 0)
	{
		self animscripts\zombie_shared::DoNoteTracksForTime(climbingTime, "climbanim");
//	println ("elapsed ", (gettime() - timer) * 0.001);
		self setFlaggedAnimKnoballRestart("climbanim",endAnim, %body, 1, .1, 1);
		self animscripts\zombie_shared::DoNoteTracks("climbanim");
	}

	self animscripts\traverse\zombie_shared::TraverseStopRagdollDeath();

	self traverseMode("gravity");
	self.a.movement = "run";
	self.a.pose = "crouch";
	self.a.alertness = "alert";
	self setAnimKnobAllRestart( animscripts\zombie_run::GetRunAnim(), %body, 1, 0.1, 1 );
	//("ladder_up: all done");#/
}
