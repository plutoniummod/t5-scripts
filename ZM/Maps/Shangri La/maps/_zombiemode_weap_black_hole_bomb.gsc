	...
	while( true )
	{
		...
		if( IsDefined( grenade ) )
		{
			...
			if( isDefined( grenade ) )
			{
				...
				model.angles = grenade.angles;
				grenade._black_hole_bomb_player = self; // saves who threw the grenade, used to assign the damage when zombies die
				...
				if(valid_poi)
				{
					...
					grenade create_zombie_point_of_interest( max_attract_dist, num_attractors, 0, true, level.black_hole_bomb_poi_initial_attract_func, level.black_hole_bomb_poi_arrival_attract_func );
...
black_hole_bomb_initial_attract_func( ent_poi )
{
	...
	while( IsDefined( ent_poi ) )
	{
		...
		{	
			// plays the fx on their back of getting pulled in
			// commented out until new anims
			//if( self._black_hole_bomb_being_pulled_in_fx == 0 )
			//{
				//self black_hole_bomb_being_pulled_fx();
			//}
...
black_hole_bomb_arrival_attract_func( ent_poi )
{
	...
	self black_hole_bomb_event_horizon_death( soul_spark_end, ent_poi );
...
black_hole_bomb_trigger_monitor( ent_trigger )
{
	...
	while( 1 )
	{
		...
		if( IsPlayer( ent_player ) && !ent_player IsOnGround() && ent_player.lander == false )
		{
