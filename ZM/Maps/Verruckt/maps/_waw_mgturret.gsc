...
#include maps\_waw_utility; 
#include common_scripts\utility;
#include common_scripts\waw_utility; 
#using_animtree( "waw_generic_human" ); 
...
mgTurret_auto( trigger )
{
	...
	maps\_waw_spawner::kill_trigger( trigger ); 
}
...
burst_fire( mg42, manual_target )
{
	...
	{
		mg42_delay = maps\_waw_mgturret::burst_fire_settings( "delay" ); 
	}
	...
	{
		mg42_delay_range = maps\_waw_mgturret::burst_fire_settings( "delay_range" ); 
	}
	...
	{
		mg42_burst = maps\_waw_mgturret::burst_fire_settings( "burst" ); 
	}
	...
	{
		mg42_burst_range = maps\_waw_mgturret::burst_fire_settings( "burst_range" ); 
	}
...
_spawner_mg42_think()
{
	...
	while( 1 )
	{
		...
		if( IsDefined( ai ) )
		{
			...
			ai thread maps\_waw_spawner::go_to_node( node ); 
			ai waittill( "death" ); 
		}
		else
		{
...
//	self endon( "auto_ai" ); 
//
////	maps\_waw_utility::debug_message( "MANUAL", self.origin ); 
//	
//	self.pacifist = true; 
...
//	maps\_waw_utility::structarray_swaptolast( level.drones[drone.team], drone ); 
...
//		assertex( IsDefined( node.turret ), "Cover node at " + node.origin + " has no turret!" ); 
	...
	#/
		...
		{
			thread maps\_waw_mgturret::turret_think( node ); 
		}
		*/
...
//	self.goalradius = 350; 
	...
	self set_all_exceptions( animscripts\waw_init::empty ); 
}
...
leave_gun_and_run_to_new_spot( spot )
{
	...
	self StopUSeturret(); 
	self animscripts\waw_shared::placeWeaponOn( self.primaryweapon, "none" ); 
...
run_to_new_spot_and_setup_gun( spot )
{
	...
	self animscripts\waw_shared::placeWeaponOn( self.weapon, "none" ); 
	if( self.team == "axis" )
	{
		self.health = 1; 
	}
	...
	self set_all_exceptions( animscripts\waw_combat::exception_exposed_mg42_portable ); 
...
//	self waittill( "runto_arrived" ); 
	...
	self animscripts\waw_shared::placeWeaponOn( self.primaryweapon, "right" ); 
...
turret_user_moves()
{
	...
	maps\_waw_mg_penetration::gunner_think( turret_node.turret ); 
}
...
#/
	...
	anim_ent maps\_waw_anim::SetAnimTree();
