...
main()
{
	...
	for (i=0;i<miniBossSpawners.size;i++)
	{
		...
		switch (struct.type)	//if you change this order, be sure to update the devgui challenges
		{
			case "sergei":
				struct.round 	= 9;
				struct.hp_mod = 0.4;
	...
	//pigs don't have any special spawner	
	struct 					= spawnstruct();
	struct.round 		= 25;
	...
	//fix up some references; The moss guys have two versions, big and small	
...
boss_battle_challenge() //self == the big boss.
{
	self endon("death");
	health = self.health;
...
barrel_bomb_think()
{
		...
		self.team = "axis"; 
		self.is_zombie = true; ///hax to fake out the player_damage_override
...
barrel_burst(which)
{
	...
	if ( flag(which) )//already bursting
		return;
