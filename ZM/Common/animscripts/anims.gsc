...
animArrayInternal( anim_array, animType, animScript, animPose, animWeaponAnims, animName, errorIfMissing, globalArrayLookup )
{
	...
	if( !IsDefined( animType_array ) )
	{
		...
		if( errorIfMissing )
		{
			...
			assertex( IsDefined( animType_array ), errorMsg );
		}
		#/
	...
	if( !IsDefined( script_array ) )
	{
		...
		if( errorIfMissing )
		{
			...
			assertex( IsDefined( script_array ), errorMsg );
		}
		#/
	...
	if( !IsDefined( pose_array ) )
	{
		...
		if( errorIfMissing )
		{
			...
			assertex( IsDefined( pose_array ), errorMsg );
		}
		#/
	...
	if( !IsDefined( weapon_array ) )
	{
		...
		if( (!IsDefined(theAnim) || (!IsArray(theAnim) && theAnim == %void)) && errorIfMissing )
		{
			...
			assertex( IsDefined(theAnim) && theAnim != %void, errorMsg );
			#/
		}
	...
	if( !IsDefined( theAnim ) )
	{
		...
		else if( IsDefined(self.coverNode) && animScript != "combat" ) // is in cover script, try exposed
		{
