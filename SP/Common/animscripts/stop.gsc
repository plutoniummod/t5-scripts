...
main()
{
	...
	/#
	if( self is_civilian() || IsDefined( self.is_sergei ) || ( IsDefined( self.is_prisoner ) && self.is_prisoner ) )
		PrintLn( "ERROR: Civilian/Sergei/Prisoner system doesnt support idling, he should be playing a scripted animation or running." );
	#/
...
// SUMEET_TODO ( 9/12/2010 ) - Ideally, there is no need to check IsInCombat() for CQB transitions as CQB transitions have proper stop
// idles. Considering late in project, just adding another variable to skip this check
// TODO is to remove this hack on the next project
IsForcedCqbStopIdle()
{
	if( IsDefined( self.a.forceCqbStopIdle ) && self.a.forceCqbStopIdle )
		return true;
...
transitionToIdle( pose, idleSet )
{
	...
	special = "";
	if( self isCQBWalking() && self.a.pose == "stand" && ( self IsInCombat() || self IsForcedCqbStopIdle() ) )
	{
		special = "_cqb";
	}
	...
	if ( animArrayExist("idle_trans" + special) )
	{
		// TODO: do a gas weapon check to stop trans in		
		if (!self usingGasWeapon() )
		{
			idleAnim = animArray("idle_trans" + special);
