	...
	{	
		...
		for(l = 0; l < attachment_categories.size; l++ )
		{
			...
			for(j = 0; j < primary_weapon_attachments[attachment_categories[l]].size; j++)
			{
				...
				{
					anim.droppable_weapons[base_weapon][attachment_categories[l]] = array_add(anim.droppable_weapons[base_weapon][attachment_categories[l]], new_weapon);
				}
			}
		}
		...
		for(j = 0; j < primary_weapon_alts.size; j++)
		{
			...
			if(IsAssetLoaded("weapon", new_weapon))
			{
				//anim.droppable_weapons[base_weapon]["alt_weapon"] = array_add(anim.droppable_weapons[base_weapon]["alt_weapon"], new_weapon);
...
get_weapon_name_with_attachments(weapon_name)
{
	...
	{
		AssertEX( level.rw_attachments_allowed, "Trying to give an attachment that is not valid.  Talk to GLocke if you hit this assert." );
