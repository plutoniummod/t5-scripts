...
canSideStep()
{
	...
	if( (!self usingRifle() && !self usingPistol()) || IsDefined(self.baton_guard) )
	{
		return false;
	}
...
trySideStep()
{
	...
	assertex( IsDefined(self.stepAnim), "rusher anim " + animName + " not found" );
...
doSideStep()
{
	...
	// enable turns
	self thread animscripts\move::turnIfNecessary();
...
sideStepBlendOut( animLength, animName, hasExitAlign )
{
	...
	runBlendInTime = 0.0; // to fix an extra frame of sliding that happens on the client
...
allowEvasiveMovement()
{
	...
	else if( IsDefined(self.isWounded) && self.isWounded ) // wounded guys aren't agile anymore
	{
		return false;
	}
...
checkRoomForAnim( stepAnim )
{
	...
	{
		/#
		recordLine( self.origin, getAnimEndPos(stepAnim), (1,0,0), "Pathfind", self );
		#/
	...
	/# 
	recordLine( self.origin, getAnimEndPos(stepAnim), (0,1,0), "Pathfind", self );
	#/
