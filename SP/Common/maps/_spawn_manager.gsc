...
trigger_spawn_manager_setup()
{
	...
	// There are only three triggers that can be used as spawn triggers. TRIGGER_SPAWN FLAG
	// Those are, "trigger_multiple", "trigger_once", "trigger_radius".
	// If a trigger needs to be used as spawn manager triggers then, it has to be one of them.
	...
	// SUMEET_TODO : if needed we can add spawn manager functionality to these triggers later on.
	// trigger_types[trigger_types.size] = "trigger_use";  
	// trigger_types[trigger_types.size] = "trigger_damage";
...
spawn_manager_setup()
{
	...
	{
		// TODO: I'm thinking this should be set to the total of all spawners active count.


		// get the number of spawners targetted by this spawn manager.
		self.sm_active_count = self.allSpawners.size;

		if (self.sm_active_count < self.sm_group_size_max)
		{
			// if the max group size is bigger than the spawner count, use the max group size instead
	...
	while( self.spawnCount < self.count && self.spawners.size > 0 )
	{
		...
		while( !spawned )
		{
			...
			if(self spawn_manager_can_spawn(self.sm_group_size))
			{
				...
				{
					// no a single spawner found for the given sm_group_size	
