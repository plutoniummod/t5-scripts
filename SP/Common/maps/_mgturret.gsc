...
{
	/#
	debug_replay("File: _mgturret.gsc. Function: main()\n");
	#/
    ...
    /#
	debug_replay("File: _mgturret.gsc. Function: main() - COMPLETE\n");
	#/
}
...
//				temp_vec = vector_scale( temp_vec, mindist ); 
...
//			for( i = 0; i < 1; i+= dif )
//			{
//				targ_org.origin = vector_scale( current_org, 1.0-i ) + 
//				vector_scale( level.player GetOrigin() + stance_num(), i ); 
//
//				if( player_safe() )
//				{
//					i = 2; 
...
find_different_way_to_attack_last_seen_position( ent )
{
	...
	for( i = 0; i < usable_spots.size; i++ )
	{
		...
		thread linetime( usable_spots[i].origin + offset, usable_spots[i].origin + offset + vector_scale( forward, 1000 ), ( 1, 0, 0 ), 10 ); 
