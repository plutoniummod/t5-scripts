...
main()
{
	...
	// DSL - added look at script.
	self LookAtEntity();	// Clear the lookatentity on this ent, to stop dead bodies looking at things.
...
waitForRagdoll( time )
{
	...
	{
		/# recordEntText( "death ragdoll", self, (0,1,0), "Anim" ); #/
		self StartRagDoll(); 
	}
...
force_extended_death_anim( value )
{
	...
	AssertEx( anims.size > 0, "There is no extended death animations present for " + self.a.pose );
	...
	{
		AssertEx(    value == "flameA_start"   || value == "flameB_start"
		...
		AssertEx( IsDefined( random_anim ), "The animation for " + self.a.pose + " " + value + "does not exist."  );
	}
...
#/	
	...
	switch( self.a.special )
	{
		...
		case "cover_pillar":
			...
			if( ( self.damageyaw > -135 ) &&( self.damageyaw <= -45 ) )	// Left quadrant
			{
				deathArray[deathArray.size] = animArrayPickRandom("cover_pillar_left");
			}
			if( ( self.damageyaw < 135 ) &&( self.damageyaw > 45 ) )	// Right quadrant
			{
		...
		case "dying_crawl":
		 ...
		 case "bleeder_death":
			if( self.a.pose == "back" )
			{
				// If the AI is not fully into back pose yet, (which should not happen ideally) then it will not do the special revive death.
				// AI_TODO - death animations specific to bleeder.
				deathArray = animArray("crawl");
				DoDeathFromArray( deathArray ); 
				return true;
			}
...
getStandDeathAnim()
{
	...
	else
	{
		...
		if( ( self.damageyaw > 135 ) ||( self.damageyaw <= -135 ) ) // Front quadrant
		{
			...
			{	
				// SUMEET - added this so that we can disable firing deaths in certain cases.
...
#/
	...
	//if(	IsDefined(self.noExplosiveDeathAnim) && self.noExplosiveDeathAnim )
	//{
	//	gib_chance = 100;
	//}
...
// AlexP ( 7/16/2010 ): New gas death animations.
play_gas_death_anim() 
{
	if ( !is_mature() )
	{
...
play_bulletgibbed_death_anim()
{
	...
	if( RandomInt( 100 ) < gib_chance && distSquared < maxDist*maxDist && (force_gib || GetTime() > anim.lastGibTime + anim.gibDelay) )
	{
		...
		// Aliu (2/22/10) Allows script to customize gib parts
...
/#
	if( GetDvar( #"scr_forceExtendedDeath" ) != "" )
		Record3DText( "AI is going to ragdoll", self.origin + ( 0, 0, 70 ), ( 1, 1, 1 ), "Script" );
#/
...
/#
	if( GetDvar( #"scr_forceExtendedDeath" ) != "" )
		Record3DText( "AI is going to play actual death", self.origin + ( 0, 0, 70 ), ( 1, 1, 1 ), "Script" );
#/
...
launch_ragdoll_based_on_damage_type( bullet_scale )
{
	...
	/# recordEntText( "death launch ragdoll", self, (0,1,0), "Anim" ); #/
...
playDeathSound()
{
	...
	else // SRS 7/9/2008: added crunchy headshot sounds
	{
