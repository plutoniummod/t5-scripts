...
shootEnemyWrapper_normal()
{
	...
	/#
	if(IsDefined(self.treyu_shoot_notify))
	{
		self notify( self.treyu_shoot_notify );
	}
	#/
...
//			{
//				/#
//				record3dtext( "left", lookFromPoint, (1,0,0), "Script" );
//				recordLine( lookFromPoint, point, (1,0,0), "Script", self );
//				#/
//			}
//			else
//			{
//				/#
//				record3dtext( "right", lookFromPoint, (0,1,0), "Script" );
//				recordLine( lookFromPoint, point, (0,1,0), "Script", self );
//				#/
//			}

			if( canSeePointFromExposedAtNodeWithOffset(point, node, lookFromPoint) )
			{
				//recordLine( lookFromPoint, point, (0,0,1), "Script", self );

				// todo: set node to use this dir
				if( nodeOffsets[i][0] < 0 )
				{
...
//	}
	...
	{
		/#
		self animscripts\debug::debugPushState( "idleLookatBehavior", "StopLookAtPlayerTest" );
		#/

		if(self animscripts\utility::IsInCombat() || !IsOkToLookAtEntity())
		{
			/#
			self animscripts\debug::debugPushState( "idleLookatBehavior", "StopLookAtPlayer" );
			#/
...
//			break;
		...
		else if((player_dist < dist_thresh) && !looking && dot_check_passed)
		{
			...
			/#
			self animscripts\debug::debugPushState( "idleLookatBehavior", "LookAtPlayer" );
			#/
