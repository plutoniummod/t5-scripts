...
getBalconyNode()
{
	...
	if (!IsDefined(balcony_node))
	{
		...
		if (!nodes.size)
		{
			debug_balcony("No potential balcony nodes.");
		}
		#/
...
check_ang_and_dist_to_node(node)
{
	...
	if (dist <= MAX_DIST)
	{
		...
		if (ang_diff <= MAX_ANGLE)
		{
			...
			/#debug_balcony_line( node.origin, self.origin, (1, 1, 0), 20);#/
...
debug_balcony_line( start, end, color, duration )
{
