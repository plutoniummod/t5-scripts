...
level thread grandfather_clock();
...
grandfather_clock()
{
	...
	sound_ent = spawn( "script_origin" , (-36030.2,-12492.6,-484.5));
	sound_ent playloopsound( "evt_grandfather_clock" );
	level waittill( "slow_mo_start" );
	player playloopsound( "amb_trippy" , (.1) );
	sound_ent stoploopsound();
	wait_network_frame();
	sound_ent playloopsound( "evt_grandfather_clock_slow" );
	level waittill( "slow_mo_stop" );
	sound_ent stoploopsound(.05);
	wait .1;
	player stoploopsound(.1);
	sound_ent playloopsound( "evt_grandfather_clock2" );
}