...
do_nag_loop( name, dialogue, ender_flag, repeat_interval )
{
	...
	level thread add_dialogue_line( name, dialogue );
	x = 0;
...
{
	trace_start = self.origin + vector_multiply((0, 0, 1), 30);
...
khe_sanh_objectives( jumpto_objective )
{
...
*/
	...
	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_HUDSON_SAFETY" );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW" );
	Objective_Position(obj_number, level.squad["woods"]);
	...
	obj_number++;
	primary_obj = obj_number;
	Objective_Add( primary_obj, "current", &"KHE_SANH_OBJ_DEFEND_KHE" );
	

	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_WOODS_CLEAR_TRENCH" );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW" );
	Objective_Position(obj_number, level.squad["woods"]);
	...
	obj_number++;
	Objective_Add( obj_number, "active", "" );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW" );
	Objective_Position(obj_number, level.squad["woods"]);
	...
	obj_number++;
	level.fougasse_obj_num = obj_number;
	obj_fougasse_one = getstruct("struct_obj_fougasse", "targetname");
	obj_fougasse_two = getstruct("struct_obj_fougasse_two", "targetname");	
	
	level.player SetClientDvar("cg_objectiveIndicatorNearDist", 32);
	Objective_Add( level.fougasse_obj_num, "active", "" );//KHE_SANH_OBJ_FOUGASSE_TRAPS
	Objective_Set3D( level.fougasse_obj_num, true, "default", "" );//KHE_SANH_OBJ_3D_DETONATOR
	Objective_AdditionalPosition( level.fougasse_obj_num, 0, obj_fougasse_one.origin );
	Objective_AdditionalPosition( level.fougasse_obj_num, 1, obj_fougasse_two.origin );

	//FOLLOW WOODS TO CHINA
	flag_wait("obj_hold_the_line");
	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_HOLD_THE_LINE" );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW" );
	Objective_Position(obj_number, level.squad["woods"]);
	...
	Objective_State( level.fougasse_obj_num, "done" );
	Objective_Delete( level.fougasse_obj_num );

	//TANK OBJECTIVE
	flag_wait("obj_defeat_the_tanks");
	Objective_State( obj_number, "done" ); //completes hold the line objective and follow
	Objective_Delete( obj_number );
	...
	Objective_String( obj_number, &"KHE_SANH_OBJ_DEFEAT_T55", tank );
	...
	if(IsDefined(level.e3_t55_tank))
	{
		...
		level thread event3_show_tank_poi(obj_number);

		//Objective_Position(obj_number, obj_law_struct.origin);
		
		obj_number++;
		Objective_Add( obj_number, "active", "" );
		//studio note to not have breadcrumb here: changed mind
		Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_USE" ); //KHE_SANH_OBJ_3D_USE
		//Objective_Position(obj_number, obj_law_struct.origin); //clearing this inside event 3 setup_law_objective()
		Objective_AdditionalPosition( obj_number, 0, obj_law_struct.origin );
		Objective_AdditionalPosition( obj_number, 1, obj_law_struct_b.origin );
		Objective_AdditionalPosition( obj_number, 2, obj_law_struct_c.origin );
		level thread cleanup_law_obj(obj_law_struct, obj_law_struct_b, obj_law_struct_c, obj_number);
			...
			if(level.e3_t55_tank.size > 0)
			{
				...
				for(i = 0; i < level.e3_t55_tank.size; i++)
				{
					if(!IsAlive(level.e3_t55_tank[i]) && !IsDefined(level.e3_t55_tank[i].imdead))
					{
						...
						Objective_AdditionalPosition( (obj_number - 1), i + 1, (0,0,0));//clear
						tank++;
						Objective_String( (obj_number - 1), &"KHE_SANH_OBJ_DEFEAT_T55", tank );
					}
				}
			}
	...
	//destroy tank obj
	Objective_State( (obj_number - 1), "done" ); 
	Objective_Delete( (obj_number - 1) ); 
	//use law obj
	Objective_State( obj_number, "done" );
	Objective_Delete( obj_number );
	...
	Objective_State( obj_number, "done" ); //completes rally bunker follow
	Objective_Delete( obj_number );	

	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_BREAKTHROUGH" ); //push back the NVA
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW" );
	Objective_Position(obj_number, level.squad["woods"]);
	...
	Objective_AdditionalPosition( obj_number, 1, down_hill.origin );

	flag_wait("trig_e4_breadcrumb");
	Objective_AdditionalPosition( obj_number, 1, (0, 0, 0) );
	remove_drone_struct(down_hill);
	Objective_AdditionalPosition( obj_number, 2, hill_trans.origin );
	
	flag_wait("obj_breakthrough_complete");
	Objective_State( (obj_number - 1), "done" ); //clears push back the nva
	Objective_State( obj_number, "done" );

	// RALLY WITH WOODS
	flag_wait("obj_rally_at_hill");
	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_RETAKE_HILL" );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW"  );
	//Objective_Position(obj_number, level.squad["woods"]);
	hilltop_woods = obj_number;
	Objective_AdditionalPosition( hilltop_woods, 1, level.squad["woods"] );
	...
	Objective_Add( obj_number, "active", "" );//KHE_SANH_OBJ_RETAKE_HILL
	Objective_Set3D( obj_number, true, "default", "" );
	Objective_AdditionalPosition( obj_number, 1, obj_flank_struct.origin );

	flag_wait("obj_retake_the_hill_rally");
	Objective_AdditionalPosition( obj_number, 1, (0,0,0) );
	Objective_Set3D( obj_number, true, "default", "" );
	Objective_AdditionalPosition( obj_number, 2, obj_hill_struct.origin );
	...
	Objective_State( obj_number, "done" );
	Objective_Delete( obj_number );	
	Objective_AdditionalPosition( hilltop_woods,  1, (0,0,0) );

	flag_wait("hilltop_window_start");

	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_SAVE_WOODS" );
	...
	Objective_AdditionalPosition( hilltop_woods, 2, level.squad["woods"] );
	Objective_Set3D( hilltop_woods, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW");
	
	//EVENT 5
	flag_wait("obj_repel_infantry");
	Objective_State( (obj_number - 1), "done" );
	Objective_Delete( (obj_number - 1) );
	Objective_State( hilltop_woods, "done" );
	Objective_Delete( hilltop_woods );


	obj_number++;
	Objective_Add( obj_number, "active", "" ); //KHE_SANH_OBJ_REPEL_ASSAULT
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_FOLLOW"  );
	//Objective_Position(obj_number, level.squad["woods"]);
	Objective_AdditionalPosition( obj_number, 1, level.squad["woods"] );

	flag_wait("obj_repel_infantry_complete");

	//clear out woods follow
	Objective_AdditionalPosition( obj_number, 1, (0,0,0) );
	...
	obj_number++;
	Objective_Add( obj_number, "current", &"KHE_SANH_OBJ_GET_JEEP" );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_ENTER" );
	Objective_Position(obj_number, level.jeep_tow GetTagOrigin("tag_passenger2") + (0, 0, 25));
	...
	last_tank = 0;
	Objective_Add( obj_number, "current", "" );
	Objective_String( obj_number, &"KHE_SANH_OBJ_REPEL_ASSAULT", last_tank );
	Objective_Set3D( obj_number, true, "default", &"KHE_SANH_OBJ_3D_DESTROY" );

	if(IsDefined(level.e5_jeep_tanks))
	{
		flag_wait("use_the_tow");
		//create objective markers on tanks
		Objective_AdditionalPosition( obj_number, 1, level.e5_jeep_tanks[0].obj_loc );
		//Objective_AdditionalPosition( obj_number, 2, level.e5_jeep_tanks[1].obj_loc );
			...
			if(level.e5_jeep_tanks.size > 0)
			{
				...
				for(i = 0; i < level.e5_jeep_tanks.size; i++)
				{
					if(!IsAlive(level.e5_jeep_tanks[i]) && !IsDefined(level.e5_jeep_tanks[i].imdead))
					{
						...
						Objective_AdditionalPosition( (obj_number), i + 1 , (0,0,0));//clear
						last_tank++;
						Objective_String( obj_number, &"KHE_SANH_OBJ_REPEL_ASSAULT", last_tank );
					}
				}
		...
		//create objective markers on tanks
		Objective_AdditionalPosition( obj_number, 2, level.e5_jeep_tanks[1].obj_loc );
		Objective_AdditionalPosition( obj_number, 3, level.e5_jeep_tanks[2].obj_loc );
		//Objective_AdditionalPosition( obj_number, 5, level.e5_jeep_tanks[4].obj_loc );
		...
		//wait until player is on tow to enable
		flag_wait("use_the_tow");
		Objective_AdditionalPosition( obj_number, 4, level.e5_jeep_tanks[3].obj_loc );
		Objective_AdditionalPosition( obj_number, 5, level.e5_jeep_tanks[4].obj_loc );
		Objective_AdditionalPosition( obj_number, 6, level.e5_jeep_tanks[5].obj_loc );
		//Objective_AdditionalPosition( obj_number, 4, level.e5_jeep_tanks[8].obj_loc );
	...
	Objective_State( (obj_number - 3), "done" );
	Objective_State( obj_number, "done" );
	Objective_Delete( (obj_number - 3) );
	Objective_Delete( obj_number );

/*
	obj_number++;
	finale_loc = getstruct("struct_finale_obj", "targetname");
	Objective_Add( obj_number, "current", "" );
	Objective_Set3D( obj_number, true, "default", "" );
	Objective_Position(obj_number, finale_loc.origin + (0, 0, 20));
*/
	flag_wait("end_scene");
//	Objective_State( obj_number, "done" );
	Objective_State( primary_obj, "done" );

}

event3_show_tank_poi(obj_num)
{
...
cleanup_law_obj(strucobj, strucobj_b, strucobj_c, num_of_obj)
{
	flag_wait("picked_up_law");
...
setup_drone_yells(all_drones, interval_wait, wait_min, wait_max, vox_weight_min, vox_weight_max, vox_weight_median)
{
	...
	{
		AssertEx((IsInt(interval_wait) || IsFloat(interval_wait)), "interval_wait must be an int or float");
	}
	...
	{
		AssertEx(IsInt(vox_weight_min), "vox_weight_min is not an int");
		AssertEx(IsInt(vox_weight_max), "vox_weight_max is not an int");
		AssertEx(IsInt(vox_weight_median), "vox_weight_median is not an int");
		AssertEx(vox_weight_min < vox_weight_max, "vox_weight_min must be < vox_weight_max");
	}
	else
	{
		vox_weight_min = 0;
	...
	{
		AssertEx(wait_min < wait_max, "wait_min must be < wait_max");
	}
	else
	{
		wait_min = 1;
...
no_look_delete(delete_time)
{
	...
	{
		AssertEx(IsInt( delete_time ), "delete_time must be an int");
	}
