...
init_flags()
{
	...
	flag_init("start_strength_test");
	flag_init( "choke_strength_test_half" );
	flag_init( "choke_strength_test_complete" );
...
ai_fake_shoot(origin_name)
{
	...
	AssertEx(IsDefined(origin_name), "origin not defined");
...
heli_shoot(shoot_at)
{
	...
	{	
		...
		AssertEx(self.heli.mod_pitch > 0, "mod pitch must be > 0");
		AssertEx(self.heli.mod_pitch > self.heli.default_pitch, "mod pitch must be > default");
...
event2_machine_gun_turret_dive()
{
	...
	level thread event2_strength_test();

	// get the align node
	align_node = GetEnt("align_bridge_choke", "targetname");
	...
	// spawn the NVA guy
	nva_diver = simple_spawn_single("e3_spawner_mg_leaper");
	nva_diver.dieQuietly = true; //kevin avoid death sounds
	//nva_diver.takedamage = false; 	
	nva_diver.animname = "nva_diver";
	nva_diver.ignoreall = true;
	nva_diver.ignoreme = true;
	//nva_diver.allowdeath = true;
	nva_diver magic_bullet_shield();

	// play the jump foley
	nva_diver playsound ("evt_khe_nva_hth_start");

	//depth of field
	level.player maps\createart\khe_sanh_art::attacking_vietnamese_dof();

	// play the dive
	actors = array(level.player.body, nva_diver);
	align_node anim_single_aligned(actors, "turret_dive");
	
	flag_set("start_strength_test");
	// play the choke loop
	align_node thread anim_loop_aligned(actors, "turret_choke");

	sound_org = spawn("script_origin", (0,0,0));
	sound_org playloopsound ("evt_khe_nva_hth_loop");
		
	//wait for test complete
	flag_wait( "choke_strength_test_complete" );
	//TUEY - FOR AUDIO
	...
	//level thread timescale_gren_qte();
	nva_diver stop_magic_bullet_shield();

	// punch him out!
	level.player thread event2_machine_gun_turret_dive_rumble();
	align_node anim_single_aligned(actors, "turret_success");
...
event2_machine_gun_turret_dive_rumble()
{
...
//reuse my stuff from creek_1_start
event2_strength_test()
{
	flag_wait( "start_strength_test" );
	level.player endon("death");
	...
	self thread event2_strength_test_button_prompt();
	...
	level.player thread event2_strength_test_rumble();

	//fails mission when you are idle based on int passed in.
	level.player thread event2_strength_test_fail_timer( fail_time );
	...
	if( !IsGodMode( level.player ) )
	{
		...
		{
			if( !flag( "choke_strength_test_half" ) && level.button_presses >= ( ( MAX_BUTTON_PRESSES * 0.05 ) * 0.5 ) )
			{
				flag_set( "choke_strength_test_half" );
			}
	...
	flag_set( "choke_strength_test_complete" );
}

event2_strength_test_button_prompt()
{
	self endon("end");
/*
	if (!IsDefined(level.strengthtest))
	{
		...
		level.strengthtest.x = -27;//-63
		level.strengthtest.y = -60;
		level.strengthtest.fontscale = 1.8;
		level.strengthtest.alpha = 0.7;
		if(level.console)
		{
			level.strengthtest SetText(&"KHE_SANH_PROMPT_PRESS_BUTTON");//[{+usereload}]		
		}
	}
	...
	if (!IsDefined(level.strengthtest_text))
	{
		...
		level.strengthtest_text.x = 0;
		level.strengthtest_text.y = -60;
		level.strengthtest_text.fontscale = 1.8;
		level.strengthtest_text.alpha = 0.7;
		if(level.console)
		{
			level.strengthtest_text SetText(&"KHE_SANH_PROMPT_PRESS");//"Press     repeatedly to fight back"		
		}
	}
*/
	if( !level.console )
	{
		screen_message_create( &"KHE_SANH_PROMPT_CHOKE" );
	}
	else
	{
		screen_message_create( &"KHE_SANH_PROMPT_CHOKE_FLASH" );
	}
/*
	while(1)
	{
		if(IsDefined(level.strengthtest))
		{
			if(level.console)
			{
				...
				wait 0.2;
				level.strengthtest.alpha = 1.0;
			}
		}

		if( flag( "choke_strength_test_complete" ) || !IsDefined(level.strengthtest))
		{
			break;
		}
		wait 0.05;
...
event2_strength_test_rumble()
{
	level.player endon("death");
	level endon("choke_strength_test_complete");
...
//self is level.player
event2_strength_test_fail_timer( fail_time )
{
	level.player endon("death");
	level endon("choke_strength_test_complete");
	...
	while(1)
	{
			...
			else
			{
				...
				if(fail_time <= 0)
				{
					...
					//self event1_player_breath_underwater();
					level thread maps\khe_sanh_util::custom_fade_screen_out("white", 0.25);
					wait 0.25;
