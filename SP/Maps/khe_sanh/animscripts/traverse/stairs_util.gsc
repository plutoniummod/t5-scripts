...
build_anim_array(stair_anims, stair_count)
{
	...
	while (stair_count > 0)
	{
		...
		for (i = 0; i < potential_sizes.size; i++)
		{
			...
			{
				sizes = array_add(sizes, potential_sizes[i]);
			}
		}

		// go through all remaining size and pick a random one
		assertex(sizes.size, "No potential animation for stair count.");
		
		random_size = random(sizes);
		if (random_size <= stair_count)
		{
			ret_stair_anims = array_add(ret_stair_anims, stair_anims[random_size]);
...
get_stair_count()
{
	...
	AssertEx(IsDefined(start_node.script_int), "Stair traversals must have a script_int with the number of stairs.");
