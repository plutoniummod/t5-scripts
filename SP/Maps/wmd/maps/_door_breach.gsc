...
/#
	while( isdefined( model ) )
	{
		...
		scale = 10;
		fwd = vector_scale( VectorNormalize( AnglesToForward( angles ) ), scale );
		right = vector_scale( VectorNormalize( AnglesToRight( angles ) ), scale );
		up = vector_scale( VectorNormalize( AnglesToUp( angles ) ), scale );
