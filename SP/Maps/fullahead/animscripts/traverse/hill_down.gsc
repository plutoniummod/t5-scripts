...
human()
{
	...
	rise = self.traverseStartNode.origin[2] - self.traverseEndNode.origin[2];
	...
	prevInAnim = animArray( "hill_down_in_"+level.hill_angles[prev], movement );
	prevOutAnim = animArray( "hill_down_out_"+level.hill_angles[prev], movement );
	prevAnim = animArray( "hill_down_"+level.hill_angles[prev], movement );
	nextInAnim = animArray( "hill_down_in_"+level.hill_angles[next], movement );
	nextOutAnim = animArray( "hill_down_out_"+level.hill_angles[next], movement );
	nextAnim = animArray( "hill_down_"+level.hill_angles[next], movement );
	...
	animTime = cycles * GetAnimLength( prevAnim );
	...
	self SetAnimKnobAllRestart( animscripts\run::GetRunAnim(), %body, 1, 0.1, 1 );
}
