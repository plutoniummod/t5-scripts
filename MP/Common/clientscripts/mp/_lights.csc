...
//			PrintLn("**** Added model to self.light_models");
			...
			self.light_models[clientNum] thread light_model_switch("on", self.script_light_on_model);
			self.light_models[clientNum] thread light_model_switch("off", self.script_light_off_model);
		}
	}
}
...
setup_mixer_lights()
{
	...
	{
		self.targetname = "light_off";
	}
	add_light_thread(self.mixer.lights[0], self.targetname, 0);
...
light_model_switch(notify_name, model_name)
{
	while(1)
	{
