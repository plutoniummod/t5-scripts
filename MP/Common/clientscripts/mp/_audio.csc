...
audio_init( localClientNum )
{
	...
	if( localClientNum == 0 )
	{
		...
		startSoundLoops();
		startLineEmitters();
		//thread update_player_vel (localClientNum);

		//thread snd_ninja_pro_watcher();
		thread snd_ninja_pro_checker();
		thread bump_trigger_start();
		//thread player_printer ();
...
snd_play_auto_fx( fxid, alias, offsetx, offsety, offsetz, onground, area )
{
	...
	//thread snd_play_auto_fx_area_emmiters
}
...
snd_ninja_pro_checker()
{
	while (1)
	{
		...
		{
			level.loudenemies = true;
		  //iprintlnbold ("players[0] has perk loudenemies_on");			
		}
		else
		{
			level.loudenemies = false;
		  //iprintlnbold ("players[0] does not have perk loudenemies_off");				
		}		
