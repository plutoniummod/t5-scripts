...
giveKillstreakIfStreakCountMatches( index, killstreak, streakCount )
{
	...
	if ( IsDefined( killstreak ) && isKillstreakAvailable(killstreak) && !hasAlreadyEarnedKillstreak )
	{
		...
		{
			// changed this to just be 'Get a N killstreak' because calling in the dogs is way too hard
			//if ( getKillstreakByMenuName( killstreak ) == "dogs_mp" )
			//	self thread maps\mp\_properks::earnedDogsKillstreak();
...
giveKillstreakForStreak()
{
	...
	//// now that we've given the kill streak, we want to cycle back around once we reach the highest
	//self check_running_killstreak();

	//This produces a prompt on the screen informing the player they've reached a certain threshold of kills.
	//The "%" makes sure the player only receives a prompt if their killstreak is perfectly divisble by 5.
	//if( !given && self.pers["totalKillstreakCount"] >= 10 )
	//{
	//	if( (self.pers["totalKillstreakCount"] % 5) == 0 )
	//	{
	//		self streakNotify( self.pers["totalKillstreakCount"] );
	//		if( (self.pers["totalKillstreakCount"] ) == 10 )
	//		{
	//			self playLocalSound( "mus_kls_10" );
	//		}	
	//		if( (self.pers["totalKillstreakCount"] ) == 15 )
	//		{
	//			self playLocalSound( "mus_kls_15" );				
	//		}	
	//		if( (self.pers["totalKillstreakCount"] ) == 20 )
	//		{
	//			self playLocalSound( "mus_kls_20" );
	//		}
	//		if( (self.pers["totalKillstreakCount"] ) > 20 )
	//		{
	//			self playLocalSound( "mus_kls_25" );
	//		}
	//	}
	//}
}
...
isOneAwayFromKillstreak()
{
	...
	// right now this logic is simply assuming that none of the counts for the selected
	// killstreaks will be equal to eachother
...
//check_running_killstreak() // self == player
//{
//	// the point of this function is to reset the current kill streak count once we hit the max
//	// this is to have rollover continually
//	
//	// get the highest kill streak
//	// TODO: we should store this value on the player when they spawn
//	highest_killstreak_type = "killstreak_null";
//	for( i = 0; i < self.killstreak.size; i++ )
//	{
//		if( self.killstreak[i] != "killstreak_null" )
//		{
//			highest_killstreak_type = self.killstreak[i];
//		}
//	}
//	
//	if( highest_killstreak_type == "killstreak_null" || highest_killstreak_type == "weapon_null" )
//	{
//		return;
//	}
//
//	// get the highest kill streak and the level of it
//	highest_killstreak = getKillstreakByMenuName( highest_killstreak_type );
//	highest_killstreak_level = level.killstreaks[highest_killstreak].killstreakLevel;
//	if ( self HasPerk( "specialty_killstreak" ) )
//	{
//		reduction = GetDvarInt( #"perk_killstreakReduction" );
//		highest_killstreak_level -= reduction;
//	}
//
//	// now if we're currently at the highest kill streak then we want to cycle back around, so reset the current streak
//	if( highest_killstreak_level == self.pers["cur_kill_streak"] )
//	{
//		// we're at the highest kill streak so reset the current streak value
...
//	if ( isDefined( self.selectingLocation ) && hasKillstreakEquipped( ) )
//	{
