...
// #include maps\_hud_util;	// TODO: For progress bar.  Is this supported in MP? 
...
*/	
	...
	SetDvar( "scr_tdm_timelimit", "0" );	// Keeps TDM timer from counting down // <<----------------MP-specific
...
fx_init()
{
	...
	level.exploderFunction = maps\mp\_utility::exploder_before_load;	// <<------------------------ MP-specific
	...
	level.exploderFunction = maps\mp\_utility::exploder_after_load;	// <<------------------------ MP-specific
	...
	for ( i=0; i<level.createFXent.size; i++ )
	{
		...
		{
			ent thread maps\mp\_fx::loopfxthread();	// <<------------------------ MP-specific
		}
		if (ent.v["type"] == "oneshotfx")
		{
			ent thread maps\mp\_fx::oneshotfxthread();	// <<------------------------ MP-specific
		}
		if (ent.v["type"] == "soundfx")
		{
			ent thread maps\mp\_fx::create_loopsound();	// <<------------------------ MP-specific
		}
	}
...
/* // >>------------------------ SP-specific ------------------------>>
		...
		// set the player's position so map restart doesnt move your origin in createfx
		playerPos = [];
...
//			black[i].alignY = "middle";
	...
	// player thread magic_bullet_shield();	// <<------------------------ SP-specific
...
//		}
		...
		// ARTIST_MOD: DaleM: 10/05/2009 - added chatpad-compatible keys to console actions
...
//			}
		...
		if (axisMode && level.selected_fx_ents.size > 0)
		{
			...
			// DaleM: 10/05/2009 - Changed rotation mode hot key from p to r
...
/#	
	...
	filename = "mpclientcreatefx/"+level.script+"_fx.csc";	// <<------------------------ MP-specific
	...
	{
		// TODO: Change to use path variables when SP/MP createfx script sync is done
		filename = "mpclientcreatefx/backup.csc";	// <<------------------------ MP-specific
	}
...
/#	
	...
	tab = "     ";
	// TODO: Change to use path variables when SP/MP createfx script sync is done
	filename = "mpcreatefx/"+level.script+"_fx.gsc";	// <<------------------------ MP-specific
	
	if ( autosave )
	{
		// TODO: Change to use path variables when SP/MP createfx script sync is done
		filename = "mpcreatefx/backup.gsc";	// <<------------------------ MP-specific
	}
	...
	{
		//player.myProgressBar updateBar(0.01, 1 / total_seconds_to_complete );	// TODO: Set up progress bar
		cfxprintln (file,"//_createfx generated. Do not touch!!");
		cfxprintln (file,"main()");
		cfxprintln (file,"{");
...
stop_fx_looper()
{
	...
	self maps\mp\_fx::stop_loopsound();	// <<------------------------ MP-specific
}
...
restart_fx_looper()
{
	...
	if (self.v["type"] == "loopfx")
	{
		...
		self maps\mp\_fx::create_looper();	// <<------------------------ MP-specific
	}

	if (self.v["type"] == "oneshotfx")
	{
		// new entities from copy/paste wont have a looper
		self maps\mp\_fx::create_triggerfx();	// <<------------------------ MP-specific
	}

	if (self.v["type"] == "soundfx")
	{
		// new entities from copy/paste wont have a looper
		self maps\mp\_fx::create_loopsound();	// <<------------------------ MP-specific
	}	
}
